...
public class GUI_DatVe extends javax.swing.JPanel {
    ...
    private void initComponents() {
        ...
        jPanel3.setPreferredSize(new java.awt.Dimension(276, 50));
        ...
        jPanel5.setMinimumSize(new java.awt.Dimension(924, 10));
        jPanel5.setPreferredSize(new java.awt.Dimension(954, 50));
	...
	private void btnDatVeActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnDatVeActionPerformed
		...
		// Lấy thông tin ThoiGianDi từ LichBay dựa trên MaLichBay trong VeMayBay
		VeMayBay maLichBay = dao_VeMayBay.getOneObjVeMayBay(maVe); // Lấy mã Lịch Bay từ DAO
		LocalDateTime thoiGianDi = maLichBay.getMaLichBay().getThoiGianDi(); // Lấy thời gian đi từ DAO

		// Kiểm tra thời gian đi phải sau thời điểm hiện tại ít nhất 2 tiếng
		LocalDateTime now = LocalDateTime.now();
		if (thoiGianDi.isBefore(now.plusHours(2))) {
		    JOptionPane.showMessageDialog(this, "Thời gian đi phải sau thời điểm hiện tại ít nhất 2 tiếng!", "Thông báo", JOptionPane.WARNING_MESSAGE);
		    return; // Kết thúc sự kiện nếu không hợp lệ
		}
	...
	public void duaDuLieuVaoNoiDi(JComboBox<String> comboBox) {
	    Set<String> noiDiSet = new HashSet<>(); // Sử dụng HashSet để loại bỏ trùng lặp
	    for (LichBay lichBay : listLichBay) {
	        if (noiDiSet.add(lichBay.getNoiDi())) { // Chỉ thêm nếu giá trị chưa tồn tại trong Set
	...
	public void duaDuLieuVaoNoiDen(JComboBox<String> comboBox) {
	    Set<String> noiDiSet = new HashSet<>(); // Sử dụng HashSet để loại bỏ trùng lặp
	    for (LichBay lichBay : listLichBay) {
	        if (noiDiSet.add(lichBay.getNoiDen())) { // Chỉ thêm nếu giá trị chưa tồn tại trong Set
